/*====================================================================
MODIFICACIÓN PARA INCLUIR LA FUNCIÓN LOGGER
Esta nueva versión cumple los mismos objetivos que la anterior, pero incluye el registro de datos en una SD.
Notas:
...
//Librerías para sensor, sleep, math, lectura memoria flash, watchdog y SD
#include <DHT.h>
#include <avr/sleep.h>
#include <math.h>
...
void registroDatos(int contadorFallos, int contadorIguales, unsigned long ahora, float humedad, float temperatura, float agua, int variableError) {
  ...
  archivo.print(agua);
  archivo.print(",");
  archivo.print(contadorFallos);
  archivo.print(",");
  archivo.print(variableError);
  archivo.print(",");
  archivo.print(contadorIguales);
  archivo.print(",");
  archivo.print(contadorAlta);
  archivo.print(",");
  archivo.println(estadoActual);
...
void setup() {
  ...
  archivo = SD.open("log.csv", FILE_WRITE);
  if (!archivo) { //Error en la SD
